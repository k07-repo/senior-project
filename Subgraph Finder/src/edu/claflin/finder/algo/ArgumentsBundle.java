...
/**
 * Used to supply arguments to Algorithms in a generalized manner.  The caller 
 * is required to put arguments on the bundle prior to passing it to the 
 * Algorithm's constructor.  The algorithm, then, can interpret the arguments 
 * it wants to and ignore others.  This generalizes the constructor of the 
 * abstract Algorithm object so that algorithms that take different arguments 
 * may be created without complicating the constructors of others.
 * 
 * Specifically, the bundle supports the following kinds of arguments:
 * - Enums
 * - Integers
 * - Floats
 * - Objects
 * - a special Condition object list
 * 
...
public class ArgumentsBundle {
    ...
    @Override
    public String toString() {
        for(Condition x: conditionsList){
