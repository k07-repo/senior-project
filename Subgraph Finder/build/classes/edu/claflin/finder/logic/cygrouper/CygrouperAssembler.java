...
public class CygrouperAssembler {
    ...
    public CygrouperAssembler(List<Edge> regularEdges){
        ...
        for(Edge e: regularEdges){
            convertCyfinderEdge(e.getSource().getIdentifier(),e.getDestination().getIdentifier());
        }
    ...
    private void assmbleNodes(List<CygrouperEdge> edgeList, Map map){
        ...
        map.forEach((key,value)->{
            ...
            System.out.println("String connections:"+(ArrayList<String>)value);
            System.out.println("String connections size:"+connections.size());

            for(int i = 0; i <connections.size();i++){
                System.out.println("in loop"+i+": String connection:"+(String)connections.get(i));
                currentNode.getConnectionsList().add((CygrouperNode)nameToObjectMap.get((String)connections.get(i)));
