...
public final class SIFGraphIO implements GraphReader, GraphWriter {
    ...
    public Graph parseGraph(File source, boolean undirected) {
        ...
        if (getLogger() != null) {
            getLogger().logInfo(LogLevel.VERBOSE, 
                    "GraphIO: Attempting to read SIF graph: "
        ...
        try (BufferedReader bR = new BufferedReader(new FileReader(source))) {
                ...
                if (line != null) {
                    ...
                    if (nodes.length < 2 && getLogger() != null) {
                        getLogger().logError(LogLevel.NORMAL,
                                "GraphIO: Error: malformed SIF line: " + line);
                    ...
                    for (int nodeIndex = 1; nodeIndex < nodes.length; nodeIndex++) {
                        graph.addEdge(new Edge(nodes[0], nodes[nodeIndex], nodeString[1], false));
                    }
                }
            } while (line != null);
            ...
            if (getLogger() != null) {
                getLogger().logError(LogLevel.NORMAL, 
                        "GraphIO: Error processing SIF Graph from file: "
                        + source.getAbsolutePath());
            }
            graph = null;
        } finally {
            if (graph != null && getLogger() != null) {
                getLogger().logInfo(LogLevel.NORMAL, 
                        "GraphIO: Successfully constructed SIF Graph from "
                        + "file.");
            }
            return graph;
        }
    ...
    public void writeGraph(Graph toWrite) {
        ...
        if (getLogger() != null) {
            getLogger().logInfo(LogLevel.NORMAL, 
                    "GraphIO: Attempt to log graph to file (SIF FORMAT): "
                    + toWrite.getName());
        }
        ...
        try (BufferedWriter bW = new BufferedWriter(new FileWriter(output))) {
            for (Edge edge : toWrite.getEdgeList()) {
                ...
                String line = String.format("%s %s %s", 
                        ...
                        edge.getDestination().getIdentifier());
                bW.write(line);
                bW.newLine();
                if (getLogger() != null) {
                    getLogger().logInfo(LogLevel.VERBOSE, 
            ...
            if (getLogger() != null) {
                getLogger().logInfo(LogLevel.NORMAL, "GraphIO: " + success +
                        " SIF graph to file: " + output.getAbsolutePath());
            }
        }
    }
